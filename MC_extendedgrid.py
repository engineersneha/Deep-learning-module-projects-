...
def create_state_action_dict(env,policy):
    ...
    for key in policy.keys():
        #Initialize 0 for all action values possible at that state
...
def mc (env,episodes=10000,policy=None,epsilon=0.001):
    ...
    for _ in range(episodes):
        #Returns cumulative rewards term 'G' set intially to 0
        G= 0
        ...
        for i in reversed (range(0,len(episode))):
            ...
            if not state_action in [(x[0],x[1]) for x in episode [0:i]]:
                ...
                #Define list for lambda expression to store state and all possible actions for that state
                Q_list = list (map(lambda x: x[1],Q[s_t].items()))
                #Create array of maximal action values in list
                indices = [i for i, x in enumerate (Q_list) if x==max(Q_list)]
                #Choose any one of maximal action to be maximal state-action value
                max_Q = random.choice(indices)
                #Assign maximal action value to A_star (variable used in pseudo code)
                A_star = max_Q
